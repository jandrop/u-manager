...
Open the **File Browser settings** in U-Manager and tap the **?** in the top bar for the full setup guide. In short: the host and port (`8740`) point at your server, and under **Authentication** you paste an Unraid API key. The plugin authenticates with the same API keys Unraid itself issues, not with your Unraid username and password — it's recommended to create a new, dedicated key (**Settings → Management Access → API Keys → Create API Key**) so you can revoke just this access later.
